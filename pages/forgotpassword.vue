        ...
        <div class="container has-text-centered">
          <div class="column is-4 is-offset-4">
            ...
            <div class="box" style="margin-top: 10px; color: red">
              <p class="subtitle is-5" style="color: red">
              ...
              <form>
                ...
                <div class="field" style="padding-top: 10px">
                  <div class="control has-icons-left">
                    <input
                      v-model="formReset.sodienthoai"
                      class="input is-small"
                      type="text"
                      placeholder="Nhập Số điện thoại đã đăng ký tài khoản"
                    />
                    <span class="icon is-small is-left">
...
export default {
  ...
  methods: {
    async resetPass() {
      try {
        // xem có thông tin trong csdl khớp không nếu có mới rs còn không thì báo lỗi
        const response = await this.$axios.post(
          `/api/auth/precallresetpass`,
          this.formReset
        );
        // console.log(response.data.success);
        if (response.data.success == true) {
          const result = await Swal.fire({
            title: `Bạn chắc chắn việc reset lại mật khẩu. Mật khẩu cũ sẽ bị thay đổi ?`,
            showDenyButton: true,
          ...
          if (result.isConfirmed) {
            ...
            if (response.status === 200) {
              ...
              Swal.fire({
                ...
                text: `Mật khẩu mới của bạn là ${response.data.newPassword}, không tiết lộ mật khẩu tránh các hiểm hoạ xảy ra với hệ thống`,
              });
              this.form = {};
            }
          }
          ...
          Swal.fire({
            title: "Sai thông tin",
            text: `Mật khẩu của bạn không reset được có thể do bạn nhập các thông tin trên chưa đúng. Hãy thử lại, chúc bạn may mắn !`,
          });
        }
      } catch (error) {
